...
* _all_ of Liquid's core filters
* nearly all of Jekyll's filters
* several other Ruby text-manipulation packages
* our own set of filters

[IMPORTANT]
Your Jekyll templates will still use Jekyll's exact set of filters. When we talk about LiquiDoc filters, we are referring to those used in LiquiDoc _parse_ actions only.

For your convenience, here we present at least basic documentation for the complete set.
